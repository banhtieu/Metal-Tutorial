...
simd::float4x4 math::scale(const float& x,
                           const float& y,
                           const float& z)
{
...
simd::float4x4 math::scale(const simd::float3& s)
{
...
simd::float4x4 math::translate(const simd::float3& t)
{
...
simd::float4x4 math::translate(const float& x,
                               const float& y,
                               const float& z)
{
    return math::translate((simd::float3){x,y,z});
} // translate

#pragma mark -
#pragma mark Public - Transformations - Rotate

static float RadiansOverPi(const float& degrees)
{
    return (degrees * k1Div180_f);
} // RadiansOverPi

simd::float4x4 math::rotate(const float& angle,
                            const simd::float3& r)
{
                            ...
                            const float& z)
{
    ...
    return math::rotate(angle, r);
} // rotate

#pragma mark -
#pragma mark Public - Transformations - Perspective

simd::float4x4 math::perspective(const float& width,
                                 const float& height,
                                 const float& near,
                                 const float& far)
{
...
simd::float4x4 math::perspective_fov(const float& fovy,
                                     const float& aspect,
                                     const float& near,
                                     const float& far)
{
    float angle  = math::radians(0.5f * fovy);
                                     ...
                                     const float& far)
{
    ...
    return math::perspective_fov(fovy, aspect, near, far);
} // perspective_fov

#pragma mark -
#pragma mark Public - Transformations - LookAt

simd::float4x4 math::lookAt(const simd::float3& eye,
...
simd::float4x4 math::lookAt(const float * const pEye,
...
simd::float4x4 math::ortho2d(const float& left,
                             const float& right,
                             const float& bottom,
                             const float& top,
                             const float& near,
...
simd::float4x4 math::ortho2d(const simd::float3& origin,
                             const simd::float3& size)
{
    return math::ortho2d(origin.x, origin.y, origin.z, size.x, size.y, size.z);
} // ortho2d

#pragma mark -
#pragma mark Public - Transformations - Off-Center Orthographic

simd::float4x4 math::ortho2d_oc(const float& left,
                                const float& right,
                                const float& bottom,
                                const float& top,
                                const float& near,
...
simd::float4x4 math::ortho2d_oc(const simd::float3& origin,
                                const simd::float3& size)
{
    return math::ortho2d_oc(origin.x, origin.y, origin.z, size.x, size.y, size.z);
} // ortho2d_oc

#pragma mark -
#pragma mark Public - Transformations - frustum

simd::float4x4 math::frustum(const float& fovH,
                             const float& fovV,
                             const float& near,
                             const float& far)
{
    float width  = 1.0f / std::tan(math::radians(0.5f * fovH));
    float height = 1.0f / std::tan(math::radians(0.5f * fovV));
...
simd::float4x4 math::frustum(const float& left,
                             const float& right,
                             const float& bottom,
                             const float& top,
                             const float& near,
...
simd::float4x4 math::frustum_oc(const float& left,
                                const float& right,
                                const float& bottom,
                                const float& top,
                                const float& near,
